...
def build_dict_stim2(stimfile, timefile, ep_duration, FC_ep):
    ...
    ep_length = ep_duration/30   # in msec
    ep1=0
    new=0
    ###
    #------------------------------------------
    # starting times of each stimulus
    starts = np.zeros([episodes, stims_ep]) + FC_ep*2700   # each FC episode is 2.7 sec we give in ms
    ...
    for t in times[:-1]:
        if t!='':
            ...
            starts[ep,st] = starts[ep,st]+ float(t) + (ep-new)*ep_length + ep1
            st_count+=1
        else:
